...
const App_EndPage = () => {
  ...
  const runtimeState = useMemo(() => {
    ...
    return sharedState?.end ?? {};
  }, [location.state]);

  useEffect(() => {
    if (!hasObjectValue(runtimeState)) return;
    mergeSharedProcessTabsState({ end: runtimeState });
  }, [runtimeState]);
  ...
  const progressPercent = useMemo(() => {
    const value = runtimeState?.summary?.progressPercent;
