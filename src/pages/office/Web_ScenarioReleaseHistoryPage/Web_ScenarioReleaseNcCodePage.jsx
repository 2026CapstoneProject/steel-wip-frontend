...
function mapBatchItemsToPickingRows(batchItems = []) {
  return batchItems
    ...
    .map((item, index) => ({
      ...
      estimatedTime: String(item.expectedStartTime ?? item.expectedRunningTime ?? "-"),
    }));
}
  ...
  return craneSchedule
    ...
    .map((row, index) => {
      ...
      return {
        ...
        ncCode: createMockNcCode(row?.ncCode ? row : itemDetail ?? row, index),
...
function getPickingRows(scenarioData) {
  ...
  if (craneSchedule.some((item) => String(item.action ?? "").trim() === "PICK")) {
