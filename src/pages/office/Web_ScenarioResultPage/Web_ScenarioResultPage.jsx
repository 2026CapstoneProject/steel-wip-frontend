...
export default function Web_ScenarioResultPage() {
	...
	const scenarioId =
		scenarioIdFromState ?? cachedProjectInfo.scenarioId ?? null;
	...
	const scenarioSummary = {
		scenarioId: scenarioData
			...
			: "-",
		projectName:
			scenarioData?.projectTitle || cachedProjectInfo.projectName || "-",
		productionPlanName:
			scenarioData?.scenarioTitle ||
			cachedProjectInfo.productionPlanName ||
			"-",
		shipmentDate:
			scenarioData?.scenarioDue || cachedProjectInfo.shipmentDate || "-",
		equipmentName:
			scenarioData?.lazerName || cachedProjectInfo.equipmentName || "-",
	...
	const handleSendConfirm = async () => {
		...
		try {
			...
			const msg =
				err.response?.data?.message || "시나리오 발행에 실패했습니다.";
			alert(msg);
		}
	};
