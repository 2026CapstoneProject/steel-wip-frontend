...
export default function Web_LantekAccordionItem({ row, index }) {
	...
	const spec = {
		...
		materialType: input.materialType ?? null, // ← 추가
	};
	...
	const handleResidualChange = (targetId, field, value) => {
		setResiduals((prev) =>
			prev.map((item) =>
				item.id === targetId ? { ...item, [field]: value } : item,
			),
		);
	};
				...
				<div className="flex items-center gap-6">
					...
					<div className="text-left">
						<div className="flex items-center gap-2 flex-wrap">
							<span className="font-headline font-bold text-on-surface text-[1.1rem]">
								{spec.manufacturer} {spec.material} - {spec.thickness}T /{" "}
						...
						<div className="text-sm text-on-surface-variant font-medium">
							두께: {spec.thickness}mm | 폭: {spec.width}mm | 길이:{" "}
							{spec.length}mm
						</div>
						{/* NC코드 표시 */}
						{row.ncCode && (
